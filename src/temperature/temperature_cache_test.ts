...
describe("TemperatureCache", () => {
  ...
  it("relative temperature", () => {
    const blueTemp = new TemperatureCache(Hct.fromInt(0xff0000ff))
      .inputRelativeTemperature;
    expect(blueTemp).toBeCloseTo(0.0, 3);

    const redTemp = new TemperatureCache(Hct.fromInt(0xffff0000))
      .inputRelativeTemperature;
    expect(redTemp).toBeCloseTo(1.0, 3);

    const greenTemp = new TemperatureCache(Hct.fromInt(0xff00ff00))
      .inputRelativeTemperature;
    expect(greenTemp).toBeCloseTo(0.467, 3);

    const whiteTemp = new TemperatureCache(Hct.fromInt(0xffffffff))
      .inputRelativeTemperature;
    expect(whiteTemp).toBeCloseTo(0.5, 3);

    const blackTemp = new TemperatureCache(Hct.fromInt(0xff000000))
      .inputRelativeTemperature;
    expect(blackTemp).toBeCloseTo(0.5, 3);
  });
