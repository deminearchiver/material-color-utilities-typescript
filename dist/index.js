...
var ColorSpecDelegateImpl2025 = class extends ColorSpecDelegateImpl2021 {
	surface() {
		const color2025 = DynamicColor.fromPalette({
			...
			tone: (s) => {
				...
				if (s.platform === "phone") if (s.isDark) return 4;
	...
	surfaceContainerLow() {
		const color2025 = DynamicColor.fromPalette({
			...
			palette: (s) => s.neutralPalette,
			tone: (s) => {
				if (s.platform === "phone") if (s.isDark) return 6;
	...
	surfaceContainer() {
		const color2025 = DynamicColor.fromPalette({
			...
			palette: (s) => s.neutralPalette,
			tone: (s) => {
				if (s.platform === "phone") if (s.isDark) return 9;
	...
	surfaceContainerHigh() {
		const color2025 = DynamicColor.fromPalette({
			...
			palette: (s) => s.neutralPalette,
			tone: (s) => {
				if (s.platform === "phone") if (s.isDark) return 12;
	...
	onSurface() {
		const color2025 = DynamicColor.fromPalette({
			...
			tone: (s) => {
				...
				else return DynamicColor.getInitialToneFromBackground((s$1) => s$1.platform === "phone" ? this.highestSurface(s$1) : this.surfaceContainerHigh())(s);
			},
			chromaMultiplier: (s) => {
				if (s.platform === "phone") {
					if (s.variant === Variant.NEUTRAL) return 2.2;
	...
	primary() {
		const color2025 = DynamicColor.fromPalette({
			...
			palette: (s) => s.primaryPalette,
			tone: (s) => {
				if (s.variant === Variant.NEUTRAL) if (s.platform === "phone") return s.isDark ? 80 : 40;
				else return 90;
				else if (s.variant === Variant.TONAL_SPOT) if (s.platform === "phone") if (s.isDark) return 80;
...
var DynamicScheme = class DynamicScheme {
	...
	*/
	...
	constructor(args) {
		...
		this.primaryPalette = args.primaryPalette ?? getSpec$1(this.specVersion).getPrimaryPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel);
		this.secondaryPalette = args.secondaryPalette ?? getSpec$1(this.specVersion).getSecondaryPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel);
		this.tertiaryPalette = args.tertiaryPalette ?? getSpec$1(this.specVersion).getTertiaryPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel);
		this.neutralPalette = args.neutralPalette ?? getSpec$1(this.specVersion).getNeutralPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel);
		this.neutralVariantPalette = args.neutralVariantPalette ?? getSpec$1(this.specVersion).getNeutralVariantPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel);
		this.errorPalette = args.errorPalette ?? getSpec$1(this.specVersion).getErrorPalette(this.variant, args.sourceColorHct, this.isDark, this.platform, this.contrastLevel) ?? TonalPalette.fromHueAndChroma(25, 84);
...
const spec2021$1 = new DynamicSchemePalettesDelegateImpl2021();
const spec2025$1 = new DynamicSchemePalettesDelegateImpl2025();
/**
